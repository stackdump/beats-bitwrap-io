...
The fallback path supports `wait: true` — block up to ~50s waiting for the farm to publish the .webm. The cap is intentionally well under typical MCP client transport timeouts (Claude.ai ≈ 60s, Claude Code ≈ 60-120s); going longer risks the client killing the call and surfacing a generic error that eats the CID. When the budget expires the tool still returns successfully with the CID and a hint to poll `get_render_status` for the long path. The MCP `/mcp` route clears the parent http.Server's 15s WriteTimeout via `http.NewResponseController` so the server-side hold can run that long. If the client gives up anyway, the render completes in the background and a re-invocation is idempotent.
...
- **go-pflow as a released dependency.** go.mod requires `github.com/pflow-xyz/go-pflow v0.22.0` and Gazelle's `go_deps` stages it from the module proxy as `@com_github_pflow_xyz_go_pflow`. No `bazel_dep`/`local_path_override` needed — go-pflow's own MODULE.bazel is irrelevant to beats' build. beats is the **root module**, so its `rules_go` (0.61.1) / Go SDK (1.26.0) govern the whole graph. (This used to be a `replace => ../go-pflow`, which meant CI had to check go-pflow out as a sibling at a hand-maintained `GO_PFLOW_REF`; adopting new go-pflow work is now a tag there and a `require` bump here.)
...
- `GET  /api/features` — `{rebuildQueue, genreColors, cohesionDefault}`. Frontend feature-detects ⟳ visibility; the studio reads `cohesionDefault` ("v1"|"v2", from `BEATS_COHESION_DEFAULT`) at boot so the server's generator-pipeline default governs client-side composes too.
