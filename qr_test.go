...
func TestQRClientParityContract(t *testing.T) {
	const (
		clientFile = "public/lib/share/qr.js"
		vendorFile = "public/vendor/qrcode.js"
	)
	mustContain := func(path, needle string) {
		t.Helper()
		...
		if !strings.Contains(body, needle) {
			t.Fatalf("%s missing invariant: %q", path, needle)
		}
	}
	// ECL must be Medium on both sides.
	mustContain(clientFile, "ecl = 'M'")
	// The URL shape must be `?cid=...&title=...`, same as the
	// server's qrTarget construction in decoratedIndex.
	mustContain(clientFile, "?cid=${cid}")
	mustContain(clientFile, "title=${encodeURIComponent(title)}")
	// Vendored lib is present and identifiable.
	mustContain(vendorFile, "QRCodeModel")
	mustContain(vendorFile, "Kazuhiko Arase")
}
