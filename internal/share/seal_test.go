...
func TestPutRateLimit(t *testing.T) {
	dir := t.TempDir()
	s, err := NewStore(dir, 1<<20, 3, 0) // 3 PUT/min cap for fast test
	if err != nil {
		t.Fatalf("NewStore: %v", err)
	}
	bodies := []string{
...
func TestPutGlobalRateLimit(t *testing.T) {
	dir := t.TempDir()
	s, err := NewStore(dir, 1<<20, 100, 3) // per-IP 100, global 3
	if err != nil {
		t.Fatalf("NewStore: %v", err)
	}
	bodies := []string{
...
func TestPutDiskCap(t *testing.T) {
	dir := t.TempDir()
	s, err := NewStore(dir, 80, 100, 0) // 80-byte cap — first body fits, second pushes over
	if err != nil {
		t.Fatalf("NewStore: %v", err)
	}
...
func TestIndexRebuildOnStartup(t *testing.T) {
	dir := t.TempDir()
	s1, err := NewStore(dir, 1<<20, 100, 0)
	if err != nil {
		t.Fatalf("NewStore: %v", err)
	}
	cid := computeCid([]byte(fixturePayload))
	...
	s2, err := NewStore(dir, 1<<20, 100, 0)
	if err != nil {
