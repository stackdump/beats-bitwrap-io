...
class ToneEngine {
    ...
    _createCustom(config, dest) {
        const created = [];
        const EFFECTS = new Set(['Gain', 'Filter', 'EQ3', 'Chorus', 'Tremolo', 'Compressor',
        ...
        // Tone's namespace exports are getter-only (Tone.Gain = ... throws), so
        // we can't swap individual constructors. Instead swap the whole global
        // `Tone` for a Proxy that returns construct-trapping wrappers for the
        // effect classes (capturing each `new Tone.X(...)` aux node) and passes
        // everything else (synth classes, Frequency, etc.) through untouched.
        // Reflect.construct preserves prototype/instanceof. Applied and removed
        // synchronously around the (synchronous) factory call.
        const realTone = (typeof window !== 'undefined') ? window.Tone : Tone;
        let instrument;
        if (realTone) {
            const proxy = new Proxy(realTone, {
                get(t, prop) {
                    const v = t[prop];
                    if (typeof v === 'function' && EFFECTS.has(prop)) {
                        return new Proxy(v, {
                            construct(T, args, nt) { const n = Reflect.construct(T, args, nt); created.push(n); return n; },
                        });
                    }
                    return v;
                },
